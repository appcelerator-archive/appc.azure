...
module.exports = Connector.extend({
	...
	create: function(Model, values, callback) {
		...
		Model.tableService.insertEntity(Model.tableName,
			entity,
			options,
	...
	findAll: function(Model, callback) {
		var connector = this;
		Model.tableService.queryEntities(Model.tableName,
			null,
			null,
			function findAllCallback(error, result) {
				if (error) {
					...
					callback(null, new Collection(Model, result.entries.map(function recordMapper(entity) {
	...
	save: function(Model, instance, callback) {
		...
		Model.tableService.updateEntity(Model.tableName,
			entity,
			options,
